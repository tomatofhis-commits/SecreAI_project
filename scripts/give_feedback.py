...
def generate_ai_text(prompt, config, system_instr=None, is_json=False):
    ...
    if provider == "openai":
        ...
        model_id = config.get("MODEL_ID_GPT", "gpt-5-mini")
...
def analyze_last_response(feedback_type, config, root, lang_data):
    try:
        log_msg = lang_data["ai_prompt"].get("feedback_analyze", "Analyzing...").format(type=feedback_type)
        ...
        instruction = lang_data["ai_prompt"].get("analyze_good" if feedback_type == "positive" else "analyze_bad", "")
        format_instr = lang_data["ai_prompt"].get("analyze_format", "")
...
def update_top_tags_with_ai(data, config, lang_data):
    try:
        system_instr = (
            f"{lang_data['ai_prompt'].get('profiler_role', '')}\n"
            f"{lang_data['ai_prompt'].get('profiler_rules', '')}"
        )
        user_instr = lang_data["ai_prompt"].get("profiler_instruction", "")
...
def main(feedback_type=None):
    ...
    try:
        ...
        final_msg = lang_data["log_messages"].get("feedback_applied", "Done").format(type=feedback_type)
        send_log_to_hub(final_msg)
        play_sound("down")
    except Exception as e:
        err_msg = lang_data["log_messages"].get("save_error", "Save failed: {e}").format(e=e)
