...
4. Ver 1.3.6 の注目機能
・多言語選択時のプロンプトおよびSTT（音声認識）の完全対応
  - 設定された言語に応じて適切な音声認識（STT）言語コードを動的に適用。
  - 全10言語辞書ファイルのai_promptを日本語固定から各言語ネイティブ指示文へ完全刷新。
・ゲートキーパーによる検索クエリ最適化精度の向上
  - ユーザーの元の質問の核心的な意図・固有名詞を維持し、AI提案がズレている場合は元の質問から直接検索キーワードを再構築するルールを強化。
・返答および検索まとめプロンプトの文字数制限（MAX_CHARS）の厳格化
  - 文字数制限を言語ごとの単位（「{max_chars}文字以内」「within {max_chars} characters/words」等）で厳密に反映し、冗長な前置きや長文解説を抑制。
