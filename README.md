...
### リアルタイム翻訳 (RTトランスレーター)
画面上のテキストを検知し、オーバーレイ表示でリアルタイムに翻訳します。
...
| **RTT 翻訳開始** | `http://localhost:5001/api/translate` | リアルタイム翻訳を開始（ポート 5001）。 |
| **RTT 状態確認** | `http://localhost:5001/api/status` | RTTプロセスの稼働状態を取得。 |
