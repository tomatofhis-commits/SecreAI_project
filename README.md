...
8. パフォーマンス最適化機能
ver0.9.7より、以下のパフォーマンス最適化機能が実装されています。これらの機能は自動的に有効化されており、追加の設定は不要です。

◆ APIキャッシュシステム
同じプロンプトや画像に対する重複したAPI呼び出しを防ぎます。一度取得した回答は一定時間キャッシュされ、同じ質問をした場合は即座に回答が返されます。
・メリット: API使用量の削減（コスト削減）、応答速度の大幅な向上
・キャッシュ保存先: data/api_cache/
・設定: config.jsonの「API_CACHE_ENABLED」で有効/無効を切り替え可能（デフォルト: 有効）
・キャッシュ保持期間: config.jsonの「API_CACHE_TTL_HOURS」で設定可能（デフォルト: 24時間）

◆ Tavily検索結果キャッシュ
ネット検索機能使用時、同じ検索クエリに対する重複した検索API呼び出しを防ぎます。
・メリット: 検索API使用量の削減、検索結果の高速取得
・キャッシュ保存先: data/search_cache/
・キャッシュ保持期間: 検索結果の鮮度を保つため、1時間で自動的にリフレッシュされます

◆ バックグラウンド並列処理
記憶整理やデータベース更新などの処理を並列実行することで、ユーザーの操作を妨げずにスムーズな体験を提供します。
・メリット: システム全体の反応速度向上、待ち時間の短縮
